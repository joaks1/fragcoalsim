...
main(int argc, char *argv[])
{
    ...
    run_sims(argc, argv, pis, pis_between, pis_within, write_to_stdout);
}

    void
run_sims(int argc, char *argv[], double * pis, double * pis_between, double * pis_within, bool write_to_stdout)
{
    ...
    pis = (double *)realloc(pis, howmany * sizeof(double) );
    pis_between = (double *)realloc(pis_between, howmany * sizeof(double) );
    pis_within = (double *)realloc(pis_within, howmany * sizeof(double) );
    /* double pis[howmany]; */
    /* double pis_between[howmany]; */
    /* double pis_within[howmany]; */
    while( howmany-count++ ) {
	   if( (ntbs > 0) && (count >1 ) ){
	         for( k=0; k<ntbs; k++){ 
    ...
    if (write_to_stdout) {
        ...
        for (unsigned int i = 1; i < howmany; ++i) {
            fprintf(pf, ", %.18lf", pis[i]);
        }
        fprintf(pf, "],\n");
        fprintf(pf, "\"pi_b\": [%.18lf", pis_between[0]);
        for (unsigned int i = 1; i < howmany; ++i) {
            fprintf(pf, ", %.18lf", pis_between[i]);
        }
        fprintf(pf, "],\n");
        fprintf(pf, "\"pi_w\": [%.18lf", pis_within[0]);
        for (unsigned int i = 1; i < howmany; ++i) {
            fprintf(pf, ", %.18lf", pis_within[i]);
        }
        fprintf(pf, "]\n}\n");
    }
...
// JRO: Here's where tree gets printed
	void
