...
class TestLevel:
    ...
    def get_current_level(cls):
        l = os.environ.get("FRAGCOALSIM_TESTING_LEVEL")
        if l is None:
            return cls.FAST
        try:
            return cls.name_to_int(l)
        except:
            _LOG.warn('the value {0} for FRAGCOALSIM_TESTING_LEVEL is not recognized.'
...
def get_test_file_names():
    ...
    for path in test_file_paths:
        ...
        test = ".".join(["fragcoalsim.test", test_file_name])
        tests.append(test)
    return tests
